...
\providecommand{\hcpRedKappaNuTens}{%
  \ensuremath{(\kappa\nu\tens)}\xspace}
\providecommand{\hcpRedKappaNuParr}{%
  \ensuremath{(\kappa\nu\parr)}\xspace}
\providecommand{\hcpRedKappaNuOne}{%
  \ensuremath{(\kappa\nu\one)}\xspace}
\providecommand{\hcpRedKappaNuBot}{%
  \ensuremath{(\kappa\nu\bot)}\xspace}
\providecommand{\hcpRedKappaNuPlus}[1]{%
  \ensuremath{(\kappa\nu\plus_{#1})}\xspace}
\providecommand{\hcpRedKappaNuWith}{%
  \ensuremath{(\kappa\nu\with)}\xspace}
\providecommand{\hcpRedKappaNuTop}{%
  \ensuremath{(\kappa\nu\top)}\xspace}
\providecommand{\hcpRedKappaMixTens}{%
  \ensuremath{(\kappa{\ppar}\tens)}\xspace}
\providecommand{\hcpRedKappaMixParr}{%
  \ensuremath{(\kappa{\ppar}\parr)}\xspace}
\providecommand{\hcpRedKappaMixOne}{%
  \ensuremath{(\kappa{\ppar}\one)}\xspace}
\providecommand{\hcpRedKappaMixBot}{%
  \ensuremath{(\kappa{\ppar}\bot)}\xspace}
\providecommand{\hcpRedKappaMixPlus}[1]{%
  \ensuremath{(\kappa{\ppar}\plus_{#1})}\xspace}
\providecommand{\hcpRedKappaMixWith}{%
  \ensuremath{(\kappa{\ppar}\with)}\xspace}
\providecommand{\hcpRedKappaMixTop}{%
...
\providecommand{\hcpInfTop}{%
  \begin{prooftree*}
    \AXC{}
    \SYM{(\top)}
    \UIC{$\seq[ \cpAbsurd{x} ]{ \mathcal{G} \hsep \Gamma, \tmty{x}{\top} }$}
  \end{prooftree*}}
