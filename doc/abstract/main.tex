...
\]
Instead, we define a new relation to characterise the relation between proofs in \hcp and \cp.
\begin{definition}\label{def:hcp-gk}
  Disentanglement is described by the smallest relation $\moveMixDown$ on proof derivations closed under the rules below.
  The relation $\moveMixDown^\star$ is the reflexive, transitive closure of $\moveMixDown$.
  \begin{center}
    \resizebox{\textwidth}{!}{
    \(
    \begin{array}{lcr}
