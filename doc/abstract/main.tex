...
\begin{definition}[Reduction]\label{def:cp-reduction}
  Reductions are described by the smallest relation $\Longrightarrow$ on process terms closed under rules below.%in \cref{fig:cp-reduction}. 
  \begin{gather*}
      \begin{array}{llcll}
        \cpRedAxCut1
...
\begin{definition}[Typing judgements]\label{def:cp}
  A typing judgement $\seq[{ P }]{\tmty{x_1}{A_1}, \dots, \tmty{x_n}{A_n}}$ denotes that the process \tm{P} communicates along channels $\tm{x_1}$, \dots, $\tm{x_n}$ following protocols $\ty{A_1}$, \dots, $\ty{A_n}$. Typing judgements are derived using rules below. %in \cref{fig:cp}.
  \begin{description}
...
\begin{definition}[Typing judgements]\label{def:hcp}
  ...
  Typing judgements can be constructed using the inference rules below. %in  \cref{fig:hcp}.
  \begin{description}
    \item[Structural rules]
    \begin{gather*}
			\hccpInfAx \qquad
...
\end{definition}\noindent
%\begin{figure}[htb]
%  \centering  Each logical rule has the side condition that $\tm{x} \not\in \ty{\mathcal{G}}$.
...
\begin{definition}\label{def:cp2hcp-terms}
  \[
    \begin{array}{lcllcl}
      ...
      &  \defeq & \tm{\cpLink{x}{y}}
      &  \tm{\mtf{\cpCut{x}{P}{Q}}}
      &  \defeq & \tm{\piNew{x}{(\piPar{\mtf{P}}{\mtf{Q}})}}
      \\ \tm{\mtf{\cpSend{x}{y}{P}{Q}}}
      &  \defeq & \tm{\piBoundSend{x}{y}{(\piPar{\mtf{P}}{\mtf{Q}})}}
      &  \tm{\mtf{\cpRecv{x}{y}{P}}}
      &  \defeq & \tm{\piRecv{x}{y}{\mtf{P}}}
      \\ \tm{\mtf{\cpHalt{x}}}
      &  \defeq & \tm{\piBoundSend{x}{}{\piHalt}}
      &  \tm{\mtf{\cpWait{x}{P}}}
      &  \defeq & \tm{\piRecv{x}{}{\mtf{P}}}
      \\ \tm{\mtf{\cpInl{x}{P}}}
      &  \defeq & \tm{\cpInl{x}{\mtf{P}}}
      &  \tm{\mtf{\cpInr{x}{P}}}
      &  \defeq & \tm{\cpInr{x}{\mtf{P}}}
      \\ \tm{\mtf{\cpCase{x}{P}{Q}}}
      &  \defeq & \tm{\cpCase{x}{\mtf{P}}{\mtf{Q}}}
      &  \tm{\mtf{\cpAbsurd{x}}}
      &  \defeq & \tm{\cpAbsurd{x}}
    \end{array}
  \]
