...
\]
Note that the parallel composition $\tm{(\piPar{P}{Q})}$ is of no relevance to
the communication, yet the rule needs to inspect the structure of the
continuation of the output to be able to nest the name restrictions
appropriately in the resulting term.
...
This work differs from our approach in that \hcp is based on \emph{classical}
linear logic, and uses hypersequents to type processes.
