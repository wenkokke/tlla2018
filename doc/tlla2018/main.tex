...
calculus construct for sending: the $(\tens)$-rule has two premises, and checks
that they share no resources.
...
Worse, if we naively added an independent parallel composition as well as an
...
We introduce \cp and restate its proofs of subject reduction and progress
(\cref{sec:cp}), then introduce \hcp and give its proofs of subject reduction
and progress (\cref{sec:hcp}).
We then prove several properties relating the two. Specifically, we prove that
every \cp process is an \hcp process, that every \hcp process corresponds to
several entangled \cp processes, and that \hcp supports the same communication
protocols as \cp, and no more (\cref{sec:cp2hcp}).
...
which we have discussed in great detail in this paper.

The idea of extending linear logic to hypersequents for typing processes is not
novel.
For instance, \citet{carbone2018} extended the multiplicative-additive fragment
...
This work differs from our approach in that \hcp is based on classical linear
logic, and uses hypersequents to type processes.
Furthermore, the rules for manipulating hypersequents are different.
In \citet{carbone2018}, hypersequents can be formed only when sequents share
