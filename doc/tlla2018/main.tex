...
\begin{definition}[Environments]\label{def:cp-environments}
  ...
  \)
\end{definition}\noindent
We write $\ty{\cdot}$ for the empty environment.
\begin{figure}[htb]
...
\begin{definition}[Reduction]\label{def:hcp-reduction}
  Reductions are described by the smallest relation $\Longrightarrow$ on process terms closed under the rules below. %rules in \cref{fig:hcp-reduction}.
%\end{definition}\noindent
%\begin{figure}[htb]
  \[
    \begin{array}{llcll}
...
%  in \cref{fig:hcp}. Furthermore, each logical rule has the side condition that $\tm{x} \not\in \ty{\mathcal{G}}$.
%\end{definition}\noindent
%\begin{figure}[htb]
